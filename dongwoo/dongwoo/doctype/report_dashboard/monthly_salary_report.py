...
""", (args.start_date, args.end_date), as_dict=True)# frappe.errprint(args.start_date)
    ...
    for row in ws.iter_rows(min_row=4, max_row=ws.max_row, min_col=11, max_col=ws.max_column):
        for cell in row:
            if isinstance(cell.value, str):  # If value is a string
                try:
                    cell.value = float(cell.value.replace(",", ""))  # Convert to number
                    cell.number_format = "#,##0"  # Ensure proper comma formatting in Excel
                except ValueError:
                    pass

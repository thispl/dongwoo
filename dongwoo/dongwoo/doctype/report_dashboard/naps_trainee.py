...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    header0 = ['']
    ws.append(header0)
    header1 = ['PERSONNEL RECORD NAPS TRAINEES']
    ws.append(header1)    
    header2 = ['Prepared by']
    ws.append(header2)    
...
@frappe.whitelist()
def get_data(args):
    var = frappe.db.get_all('Employee', {'status': 'Active','employee_type':'NAPS'}, ["*"], order_by='department')
    count=0
    row=[]
    data=[]
    qual=''
    ...
    for i in var:
        chart_data = frappe.db.get_all('Organization Chart', {'employee': i.name,'employee_type':'NAPS'}, ["*"],order_by='department')

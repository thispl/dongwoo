...
""", (args.start_date, args.end_date), as_dict=True)# frappe.errprint(args.start_date)
    ...
    for category in category_order:
        ...
        if tot_emp > 0:
            ...
            if row['Etype'] == 'NAPS':
                ws.append(['', 'TOTAL FOR NAPS','TOTAL FOR NAPS', '','',*wmem,'','','','',format_currency(wgrand_total_basic), format_currency(wgrand_total_hra), format_currency(wgrand_total_med), format_currency(wgrand_total_convey), format_currency(wgrand_total_edu), format_currency(wgrand_total_lta), format_currency(wgrand_total_dress), format_currency(wgrand_total_gross),
    ...
    for rows in ws.iter_rows(min_row=1, max_row=ws.max_row, min_col=3, max_col=3):
        for cell in rows:
            ...
            if cell.value == 'Grand Total' or cell.value=='TOTAL FOR NAPS' or cell.value=='TOTAL FOR TRAINEE' or cell.value=='TOTAL FOR STAFF':
                for cell in ws[cell.row]:
                    cell.fill = yellow
                    ws.merge_cells(start_row=cell.row, start_column=2, end_row=cell.row, end_column=4) 
    ...
    for row in ws.iter_rows(min_row=4, max_row=ws.max_row, min_col=11, max_col=ws.max_column):
        for cell in row:
            if isinstance(cell.value, str) and cell.value.replace(',', '').isnumeric():
                cell.value = int(cell.value.replace(',', ''))  # Convert to integer, removing commas
            elif isinstance(cell.value, str):
                try:
                    cell.value = float(cell.value.replace(',', ''))  # Convert to float if needed
                except ValueError:
                    pass
